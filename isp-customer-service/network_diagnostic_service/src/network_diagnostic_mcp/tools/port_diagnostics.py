...
def check_port_status(db: DatabaseConnection, customer_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Port check complete: {diagnostics['ports_up']}/{diagnostics['total_ports']} ports up"
        )
        ...
        return {
            ...
            "message": (
                "Visi portai veikia teisingai" if all_ports_up else "Aptikti neaktyvūs portai"
            ),
        }
...
def get_switch_info(db: DatabaseConnection, customer_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "switch": switch_data}
