...
def check_area_outages(
    ...
    try:
        ...
        total_affected = sum(outage.get("affected_customers", 0) or 0 for outage in outages)
        ...
        return {"success": True, "outages": outages, "summary": summary, "message": message}
...
def check_customer_affected_by_outage(db: DatabaseConnection, customer_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not address:
            return {"success": False, "error": "no_address", "message": "Kliento adresas nerastas"}
        ...
        if result.get("success") and result.get("outages"):
            ...
            for outage in result["outages"]:
                ...
                if outage_type == "all" or any(
                    svc in outage_type or outage_type in svc for svc in services
                ):
