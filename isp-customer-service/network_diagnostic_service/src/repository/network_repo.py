    ...
    def get_customer_network_summary(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "recent_bandwidth": [
                    log.model_dump() 
                    for log in self.get_bandwidth_logs(customer_id, limit=5)
                ],
                "switch": self.find_switch_by_customer(customer_id).model_dump() 
                         if self.find_switch_by_customer(customer_id) else None
            }
        except Exception as e:
