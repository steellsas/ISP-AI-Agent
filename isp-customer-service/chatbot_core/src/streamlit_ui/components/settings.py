"""
Settings Tab Component with Admin Protection

Configuration for language, models, and other options.
Settings are locked by default - requires admin password to modify.
"""
...
def get_admin_password() -> str:
    # Pirma tikrina env, tada st.secrets, tada default
    return os.getenv("ADMIN_PASSWORD") or st.secrets.get("ADMIN_PASSWORD", "admin123")


def check_admin_authenticated() -> bool:
...
def render_settings_tab():
    ...
    if check_admin_authenticated():
        render_admin_settings()
    else:
        render_locked_settings()
...
def render_locked_settings():
    """Render settings in locked/read-only mode."""
    ...
    <div style="
        ...
        margin-bottom: 20px;
    ">
        <span style="font-size: 1.2em;">🔒 Settings are locked</span><br>
        <span style="color: #666;">Enter admin password to modify settings.</span>
    </div>
...
| 🌡️ Temperature | **{settings.get('temperature', 0.3)}** |
        """)
...
def render_admin_settings():
    """Render full settings panel for authenticated admin."""
    ...
    with col1:
        ...
        <div style="
            ...
            margin-bottom: 20px;
        ">
            <span style="font-size: 1.2em;">🔓 Admin Mode Active</span><br>
            <span style="color: #666;">You can modify all settings.</span>
        </div>
        """, unsafe_allow_html=True)
