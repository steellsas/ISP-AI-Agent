...
def render_active_call():
    ...
    # Process user input
    if user_input:
        # Check if max turns reached before sending
        ...
        if turn_count >= max_turns:
            session_add_message("assistant", "Sorry, the conversation limit has been reached. Please start a new call.")
            end_call()
            st.rerun()
            return
        ...
        if "error" in result:
            ...
            # Check if conversation ended (includes max_turns check)
...
def render_agent_panel():
    ...
    turn_count = decision_info.get("turn_count", 0)
    max_turns = decision_info.get("max_turns", 20)
    progress_value = min(turn_count / max_turns, 1.0)  # Clamp to max 1.0
    
    if turn_count >= max_turns:
        st.warning(f"⚠️ Conversation limit reached ({turn_count}/{max_turns})")
        st.progress(1.0, text=f"Turn: {turn_count}/{max_turns}")
    else:
        st.progress(progress_value, text=f"Turn: {turn_count}/{max_turns}")
