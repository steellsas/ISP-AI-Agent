...
def test_load_documents():
    ...
    retriever = get_retriever(top_k=3, similarity_threshold=0.5)
    ...
    for directory in directories:
        ...
        try:
            retriever.load_documents_from_directory(
                directory=dir_path, file_extension=".md", recursive=False
            )
...
def test_context_retrieval(retriever):
    ...
    try:
        context = retriever.retrieve_with_context(query, top_k=2, include_scores=True)
