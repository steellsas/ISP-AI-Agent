...
def get_problem_type_config(problem_type: str) -> dict:
    ...
    return config["problem_types"].get(
        problem_type, 
        config["problem_types"]["other"]
    )
...
def get_context_threshold(problem_type: str) -> int:
    ...
    return config.get(
        "context_threshold", 
        settings.get("default_context_threshold", 70)
    )
...
def get_max_questions(problem_type: str) -> int:
    ...
    return config.get(
        "max_questions", 
        settings.get("default_max_questions", 3)
    )
...
def get_next_question(problem_type: str, known_facts: dict, use_simpler: bool = False) -> Optional[dict]:
    """
