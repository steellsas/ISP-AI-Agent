...
def create_support_ticket(
    ...
    try:
        ...
        if troubleshooting_steps:
            steps_text = "\n".join([f"- Žingsnis {i+1}: {step}" for i, step in enumerate(troubleshooting_steps)])
            full_details += f"\n\nAtlikti troubleshooting žingsniai:\n{steps_text}"
        
        result = create_ticket(db, {
            "customer_id": customer_id,
            "ticket_type": ticket_type,
            "summary": summary,
            "details": full_details,
            "priority": priority,
            "troubleshooting_steps": str(troubleshooting_steps) if troubleshooting_steps else None
        })
