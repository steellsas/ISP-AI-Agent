...
class MCPService:
    ...
    def __init__(
        ...
        if network_server_path:
            ...
            # Default: ../network_diagnostic_service from chatbot_core root
            self.network_server_path = (
                Path(__file__).parent.parent.parent.parent / "network_diagnostic_service"
            )
    ...
    def _check_initialized(self):
        ...
        if not self.is_initialized:
            raise RuntimeError(
                "MCPService not initialized. Call await mcp_service.initialize() first."
            )
    ...
    async def call_tool(
        ...
        Routes tool calls to the appropriate MCP server based on server name.
        This is the main method used by graph nodes.

        Args:
            server_name: Server name - "crm_service", "network_diagnostic_service",
                        "crm", or "network" (case-insensitive)
            tool_name: Name of the tool to call
        ...
        if "crm" in server_name_lower:
            logger.info(f"Routing to CRM: {tool_name}")
            return await self.call_crm_tool(tool_name, arguments)

        elif "network" in server_name_lower or "diagnostic" in server_name_lower:
            logger.info(f"Routing to Network: {tool_name}")
            return await self.call_network_tool(tool_name, arguments)

        else:
            error_msg = (
                f"Unknown server: '{server_name}'. "
                f"Valid options: 'crm_service', 'network_diagnostic_service', 'crm', 'network'"
            )
    ...
    async def call_crm_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Available tools:
        - lookup_customer_by_phone (NEW - needs to be added to CRM service)
        - lookup_customer_by_address
        - get_customer_details
        - get_customer_equipment
        - create_ticket
        ...
        try:
            ...
            logger.info(f"CRM tool {tool_name} completed successfully")
            logger.info(f" result {result} completed successfully")
            ...
            error_result = {"success": False, "error": "mcp_error", "message": str(e)}

            self._log_tool_call(
                MCPServerType.CRM, tool_name, arguments, error_result, success=False
            )
    ...
    async def call_network_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Available tools:
        - run_diagnostics (NEW - aggregated, needs to be added)
        - check_port_status
        - check_ip_assignment
        - check_bandwidth_history
        - check_area_outages
        ...
        try:
            ...
            self._log_tool_call(
                MCPServerType.NETWORK, tool_name, arguments, error_result, success=False
            )
    ...
    def _log_tool_call(
        ...
        call_log = {
            ...
            "arguments": arguments,
            "success": success,
            "result_summary": {"success": result.get("success"), "error": result.get("error")},
        }
...
def get_mcp_service(
    ...
    if _mcp_service is None:
        _mcp_service = MCPService(
            crm_server_path=crm_server_path, network_server_path=network_server_path
        )
...
if __name__ == "__main__":
    ...
    async def test_mcp():
        ...
        try:
            ...
            # ========== TEST NEW call_tool() METHOD ==========

            print("\n" + "=" * 60)
            print("Testing call_tool() with CRM")
            print("=" * 60)
            result = await mcp.call_tool(
                server_name="crm_service",  # or "crm"
                tool_name="lookup_customer_by_address",
                arguments={"city": "Šiauliai", "street": "Tilžės g.", "house_number": "60"},
            )
            print(f"Result: {json.dumps(result, indent=2, ensure_ascii=False)}")

            print("\n" + "=" * 60)
            print("Testing call_tool() with Network")
            print("=" * 60)
            result = await mcp.call_tool(
                server_name="network_diagnostic_service",  # or "network"
            ...
            print("\n" + "=" * 60)
            print("Testing original call_crm_tool()")
            print("=" * 60)
            result = await mcp.call_crm_tool(
                "lookup_customer_by_address",
            ...
            print("\n" + "=" * 60)
            print("Testing original call_network_tool()")
            print("=" * 60)
            result = await mcp.call_network_tool("check_port_status", {"customer_id": "1"})
            print(f"Result: {json.dumps(result, indent=2, ensure_ascii=False)}")

            # ========== TEST ERROR HANDLING ==========

            print("\n" + "=" * 60)
            print("Testing error handling (invalid server)")
            print("=" * 60)
            try:
                result = await mcp.call_tool(
                    server_name="invalid_service", tool_name="some_tool", arguments={}
                )
            ...
            for call in recent:
                print(
                    f"  {call['timestamp']} - {call['server_type']}/{call['tool_name']} - Success: {call['success']}"
                )
