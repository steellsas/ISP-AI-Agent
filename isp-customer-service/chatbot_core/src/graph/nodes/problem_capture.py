...
def _get_translated_question(
    problem_type: str, field_key: str, use_simple: bool = False
) -> str | None:
    """
...
def _get_next_translated_question(
    problem_type: str, known_facts: dict, use_simple: bool = False
...
LANGUAGE INSTRUCTION: You MUST generate all customer-facing text (next_question, problem_summary, simpler_alternative) in {output_language} language. This is critical."""
    ...
    try:
        response = llm_json_completion(messages=messages, temperature=0.3, max_tokens=500)
        ...
        if current_error_count >= 3:
            error_message = add_message(
                role="assistant", content=t("errors.llm_error"), node="problem_capture"
            )
            return {
        ...
        fallback_message = add_message(
            role="assistant", content=t("errors.not_understood"), node="problem_capture"
        )
        return {
    ...
    should_proceed = (
        analysis.ready_to_proceed
        or analysis.context_score >= threshold
        or questions_asked >= max_q
        or analysis.user_wants_to_skip
        or intent["wants_skip"]
    )
    ...
    if should_proceed:
        ...
        message = add_message(role="assistant", content=full_message, node="problem_capture")

        logger.info(
            f"[{conversation_id}] Problem capture complete | type={analysis.problem_type} | lang={get_language()}"
        )
        ...
        # === ASK NEXT QUESTION ===
        logger.info(
            f"[{conversation_id}] Need more context - asking question #{questions_asked + 1}"
        )
        ...
        if not question_text:
            question_text, target_field = _get_next_translated_question(
                analysis.problem_type, updated_facts, use_simple=analysis.user_said_unknown
            )
            if target_field:
...
def problem_capture_router(state: Any) -> str:
    ...
    if _get_attr(state, "problem_capture_complete", False):
        ...
        logger.info(
            f"[{conversation_id}] Router → phone_lookup (complete: {problem_type}, score: {context_score})"
        )
        return "phone_lookup"
