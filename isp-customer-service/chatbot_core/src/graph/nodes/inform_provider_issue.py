...
def _build_outage_message(issue: dict) -> str:
    ...
    if estimated_resolution:
        ...
        message = t("provider_issue.area_outage", outage_type=outage_type_name)

    # Add affected customers info
    if affected_customers:
        message += "\n\n" + t("provider_issue.affected_customers", count=affected_customers)

    # Add description
    if description:
        message += "\n\n" + t("provider_issue.details", description=description)
    ...
    try:
        ...
        if not provider_issues:
            ...
            if issue_type == "area_outage":
                ...
                message_text = t("provider_issue.maintenance", end_time=end_time)
            else:
                message_text = t("provider_issue.generic")
        ...
        message = add_message(role="assistant", content=message_text, node="inform_provider_issue")
