...
from src.services.language_service import sync_language_from_state, get_agent_name, get_language
...
def greeting_node(state: Any) -> dict:
    ...
    try:
        ...
        if customer_name:
            ...
            greeting_text = t("greeting.welcome_returning", customer_name=customer_name)
            logger.info(f"[{conversation_id}] Returning customer: {customer_name}")
        else:
            # New customer - standard greeting
            greeting_text = t("greeting.welcome", company_name=company_name, agent_name=agent_name)
            logger.info(f"[{conversation_id}] New customer greeting")

        # Create message
        message = add_message(role="assistant", content=greeting_text, node="greeting")

        logger.info(
            f"[{conversation_id}] Greeting node completed | lang={lang} | agent={agent_name}"
        )

        return {"messages": [message], "current_node": "greeting"}
        ...
        message = add_message(role="assistant", content=fallback_greeting, node="greeting")

        return {"messages": [message], "current_node": "greeting", "last_error": str(e)}
