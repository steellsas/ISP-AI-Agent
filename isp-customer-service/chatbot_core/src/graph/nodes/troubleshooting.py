...
def translate_step_content(
    text: str, content_type: str = "title", conversation_id: str = ""
) -> str:
    """
...
Be friendly and patient."""
    ...
    try:
        ...
        return t(
            "troubleshooting.help_generic", step_title=step_title, instruction=step_instruction
        )
...
def generate_apology_response(
    frustration: dict, checked_items: dict, problem_description: str
) -> str:
...
def build_context_summary(problem_context: dict, problem_type: str) -> str:
    ...
    if scope:
        ...
        if any(kw in scope_lower for kw in ["visi", "visur", "visuose", "all"]):
            ...
            kw in scope_lower for kw in ["telefon", "phone", "vienas", "one", "laptop", "kompiuter"]
        ):
...
def determine_starting_step(
    ...
    if diagnostic_results:
        ...
        if ip_active and port_healthy:
            logger.info(
                f"[{conversation_id}] ✅ Diagnostics confirm router is working - can skip router checks"
            )
    ...
    if problem_context:
        ...
        if is_single_device:
            logger.info(
                f"[{conversation_id}] ✅ Single device affected: '{scope}' - skipping to device check (step 4)"
            )
    ...
    if problem_context:
        ...
        if tried_restart is True:
            logger.info(
                f"[{conversation_id}] ✅ Customer already tried restart - skip restart step"
            )
    ...
    if problem_context:
        ...
        if router_lights:
            ...
            if any(kw in lights_lower for kw in ["žalia", "green", "dega", "visos"]):
                logger.info(
                    f"[{conversation_id}] ✅ Router lights already known: {router_lights} - skip to device check"
                )
                device_step = scenario.get_step(4)
                if device_step:
                    skipped_steps = [1, 2, 3]
...
def get_smart_scenario_id(
    ...
    if problem_context and problem_type == "internet":
        ...
        if any(kw in scope for kw in single_device_keywords):
            logger.info(
                f"[{conversation_id}] ⚡ Instant routing: single device → internet_single_device"
            )
...
def analyze_user_response(
    user_response: str, current_step: dict, conversation_id: str = ""
) -> StepResponse:
    ...
    if branches and isinstance(branches, dict):
        ...
        for branch_id, branch_data in branches.items():
            ...
            condition = (
                branch_data.get("condition", "")
                if isinstance(branch_data, dict)
                else str(branch_data)
            )
...
def troubleshooting_node(state: Any) -> dict:
    ...
    if not ts_state["scenario_id"]:
        ...
        if not scenario and scenario_id != "internet_no_connection":
            logger.warning(
                f"[{conversation_id}] Scenario '{scenario_id}' not found, falling back to 'internet_no_connection'"
            )
        ...
        starting_step, skipped_steps, skip_message = determine_starting_step(
            scenario, problem_context, problem_type, diagnostic_results, conversation_id
        )
        ...
        logger.info(
            f"[{conversation_id}] Started scenario: {scenario_id} | step: {starting_step.get('step_id')} | skipped: {skipped_steps}"
        )
    ...
    if detect_help_request(user_response):
        ...
        message = add_message(role="assistant", content=help_response, node="troubleshooting")
    ...
    if frustration.get("should_escalate"):
        ...
        message = add_message(
            role="assistant", content=t("troubleshooting.offer_escalation"), node="troubleshooting"
        )
    ...
    if frustration.get("should_apologize"):
        logger.info(
            f"[{conversation_id}] Frustration detected | severity={frustration.get('severity')}"
        )

        apology_response = generate_apology_response(
            frustration, ts_state.get("checked_items", {}), problem_description
        )

        message = add_message(role="assistant", content=apology_response, node="troubleshooting")
    ...
    if any(phrase in user_response.lower() for phrase in resolution_phrases):
        ...
        if resolution.is_resolved and resolution.confidence > 0.7:
            ...
            message = add_message(
                role="assistant", content=t("troubleshooting.resolved"), node="troubleshooting"
            )
            ...
            return {
                ...
                "troubleshooting_completed_steps": ts_state["completed_steps"]
                + [ts_state["current_step"]],
            }
    ...
    if not current_step:
        ...
        error_msg = add_message(
            role="assistant", content=t("troubleshooting.step_not_found"), node="troubleshooting"
        )
        return {
            "messages": [error_msg],
            "current_node": "troubleshooting",
    ...
    if analysis.selected_branch:
        ...
        if branch_data:
            ...
            if action == "escalate" or next_step_id == "escalate":
                ...
                message = add_message(role="assistant", content=content, node="troubleshooting")

                return {
                    "messages": [message],
                    "current_node": "troubleshooting",
                    "troubleshooting_needs_escalation": True,
                    "troubleshooting_escalation_reason": branch_data.get(
                        "reason", "branch_escalate"
                    ),
                }
    ...
    if not next_step:
        ...
        message = add_message(
            role="assistant", content=t("troubleshooting.escalating"), node="troubleshooting"
        )
    ...
    message = add_message(
        role="assistant", content=f"**{step_title}**\n\n{instruction_text}", node="troubleshooting"
    )

    logger.info(
        f"[{conversation_id}] Moving to step: {next_step.get('step_id')} | lang={get_language()}"
    )
