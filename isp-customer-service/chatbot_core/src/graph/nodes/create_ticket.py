    ...
    try:
        ...
        if result.get("success"):
            ...
            if not problem_resolved:
                ...
                message_text = t("ticket.created", customer_name=first_name, ticket_id=ticket_id)

                message = add_message(role="assistant", content=message_text, node="create_ticket")
            ...
            if not problem_resolved:
                message = add_message(
                    role="assistant", content=t("ticket.creation_failed"), node="create_ticket"
                )
                messages.append(message)
