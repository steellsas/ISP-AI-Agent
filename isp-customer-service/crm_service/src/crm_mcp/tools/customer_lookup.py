...
def fuzzy_match_street(input_street: str, db_streets: List[str], threshold: float = 0.7) -> Optional[str]:
    """
...
def lookup_customer_by_address(db: DatabaseConnection, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    apartment_number = args.get("apartment_number", "").strip() if args.get("apartment_number") else None
    
    logger.info(f"Looking up customer: {city}, {street} {house_number}" + 
    ...
    try:
        ...
        logger.info(f"Found customer: {customer_data['customer_id']} - {customer_data['first_name']} {customer_data['last_name']}")
