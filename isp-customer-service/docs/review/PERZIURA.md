...
| **U** | **Diagnostika = interneto sprendimų medis kode** (`verdict.py`): gera greičiui, bet domenas kode; Case architektūrai reikia žalių signalų → faktų (telemetry.port_up, mac_seen, crc_rate…), o sąlygas deklaruoja kortelės. **Uždaryta 4a bangoje (2026-09-23):** `verdict.py::decide` ištrintas, zondas grąžina `signals`, visas 12 verdiktų vardija kortelės (6 gedimų + 8 informavimo). |
