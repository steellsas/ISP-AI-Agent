...
| F-24 | Voice latency after M5: the first audio starts only once the WHOLE reply text is finished (the graph delivers the custom stream when the node completes, so TTS no longer gets sentence by sentence); the analyst runs inside the turn because `ANALYST_MODE` defaults to `sync` (voice should use `async` — the background thread already exists); and twice the TTS itself took 8.7 s and 9.9 s after the text was ready (edge-tts), which speculation used to hide with cached audio | `graph_v2/runtime.narrate` stream writer, `analyst/node.mode`, `adapters/tts/edge_tts` | open — owner picks the order |
