...
| M5 | `speak` node + analyst signals; `ReactAgent` deleted | [M5_speak_and_analyst.md](M5_speak_and_analyst.md) | D-02, D-06, D-15 | [x] `e017988` |
...
| 2026-09-16 | M5 done (owner live voice test pending) | `a622ca0` … `e017988` | The speaker is `agent/speak/`: an English CONTEXT CARD built from typed state (`context_card.py`, every one of the 49 Lithuanian KNOWN FACTS sections mapped in `baseline/m5_card_inventory.md`), a byte-stable prompt prefix per plan owner (`prompts/speak/system.md` + `owners/*.md`), the history window (`history.py`), the turn itself with streaming and barge-in (`node.py`) and the reply guards (`postprocess.py`: the registration claim, the repeat guard and a new `reply_max_chars` cap trimmed at the last full sentence). The speaking LLM has NO tools anywhere — the engine runs every lookup (identification included), so `tool_scopes.py`, the stage prompts and `partials/directives.md` are gone and the LLM client omits `tool_choice` when a call has no tools. The analyst is `agent/analyst/`: typed signals (contradiction, already_answered, secondary_problem, off_topic, frustration) with a confidence floor, applied by the machinery that owns each decision — a contradiction becomes a doubt the caller confirms, an already-answered fact a confirm question, a secondary problem joins the closing list, tone signals reach only the card; it writes no fact and changes no step. `ANALYST_MODE=sync|async|off`, every read traced (empty included). Speculation removed (Q-2); the background telemetry refresh survives in `agent/background.py`. `ReactAgent` and `narrator_flow` deleted: the call record is `session_record.py`, the tool-result state writes `execute/observe.py`, the step presentation `execute/step.py`. F-11 fixed the way the owner chose (2026-09-16): a pack's activation seeds the ledger from the WHOLE call, and a step whose answer the caller volunteered is checked back („Supratau — visuose įrenginiuose. Ar teisingai?“), a yes taking that answer's branch — the alternative (silently skipping the step) desynchronised D8 and was reverted. pytest 1175 green, app starts. Eval 110/111 with a new `A1_repeat_and_frustration` scenario (baseline 105/108): the only failure was `T1` (F-21, passes 2/2 on a re-run). Deviations: `NARRATOR_QUESTIONS` stays until the prompt/wording pass (owner 2026-09-16 — removing it now would mean rewriting 25 tests that assert scripted sentences); the sensor prompts (`perception*`, `ticket_reader*`) stay Lithuanian (M3 finding), and `P/` otherwise holds only place names and quoted caller-facing words; `executor_flow.py` still runs the LLM tool rounds' execution path even though no LLM is offered tools (M6 removes the dead path with the tool catalog); the analyst's clarify loop back into a fresh hypothesis, the verification of what the caller actually did, and the small-step physical guidance are written up in `OWNER_NOTES_dialogue.md` and not implemented; findings F-12, F-15, F-17, F-19 (planned for M5) are untouched — the owner re-assigns them with that note's work |
...
| F-11 | ~~"Neveikia internetas visuose įrenginiuose" in the first turn is not captured: after `router_hung` the agent still asks "visuose ar tik viename?" (eval probe, CUST112) | understand / facts intake ~~ | M5 — fixed (`a68143b`, `573f4db`: the activation seeds the ledger from the whole call; a step whose answer the caller volunteered is checked back, not re-asked) |
...
| F-22 | Live call 2026-09-16 (`logs/sessions/20260916-111305`): the opening "neveikia visuose įrenginiuose" arrived garbled from STT ("nevyk internetas visose renginiuose"), so the pack's answer markers did not match and nothing landed. The fact DID land later through the normal ingest, but the check-back only covers facts marked by the activation seed, so the agent still asked "visuose ar tik viename?" | `execute/diagnosis._seed_evidence_from_call` markers, `decide/rules/evidence.seeded_step_confirm` (the `seeded` flag) | open — owner picks the order (M5 notes §1) |
| F-23 | The `router_hung` pack declares no LIGHTS evidence at all, so nothing is asked before the reboot (owner: all lights on but not blinking = hung; during the boot one lights, then all; the WAN light blinking = traffic). The verify step asks "ar interneto lemputė mirksi?" without naming WHICH light or where it is — the caller asked "apie kurią lemputę kalbat?" | `knowledge/faults/internet_pakibes_routeris.yaml` (evidence + `rh_check` hint) | open — knowledge work (M5 notes §3) |
| F-24 | Voice latency after M5 (live call 2026-09-16). Measured cause by cause: (1) the analyst ran inside the turn — fixed in `990397d` (a call with a background window reads it there); (2) edge-tts stalled 8.7 s and 9.9 s on a sentence — fixed in `990397d` (a silent socket is abandoned and reconnected; median 0.86 s, worst 3.2 s measured); (3) my first read that "audio waits for the whole reply" was WRONG — probed on the real graph, the speaker's tokens reach the voice pipeline 0.4 s apart as the LLM produces them, and TTS starts per sentence. What remains on a normal turn is ASR ~0.5 s + the perception pass ~1.2 s (the `answer` event lands 0.9-1.4 s after ASR — the understanding LLM call, everything else in decide/execute is instant) + the speaker's first sentence ~1 s + TTS ~0.5 s ≈ 3 s | `perceive/understand` (the pass before decide), `speak/node`, `adapters/tts/edge_tts` | (1)(2) fixed; the perception wait is the next lever — owner decides |
| F-25 | Second live call 2026-09-16 (`20260916-133826`): the call's FIRST sentence still took 6.5 s in TTS (the stall guard waited `tts_stall_seconds`=3, then reconnected — a cold connection at call start), and one later sentence 3.6 s; all other sentences 0.4-1.4 s | `adapters/tts/edge_tts` (no warm connection before the greeting) | open — polish after the refactor |
| F-26 | Wording seen in the same call, to polish with the prompts: the reboot check announced a future question („Tada paklausiu, ar interneto lemputė mirksi…“); the close explained a blinking internet light as „routeris gauna srovę ir ryšys atsigavo“ and closed on the light alone; `trace_note` records the successful address-offer commit („offer confirmed; engine resolve“) as an `error` event | `prompts/speak/*`, `router_hung` pack `rh_check`, `decide/rules/head.py` trace note | open — polish after the refactor |
| F-27 | The end-confirm question asks two things in one breath („Ar tikrai norite baigti pokalbį? Jei norite, galiu užregistruoti gedimą…“), so a caller's „Taip, baigiam“ was read as a yes to REGISTERING and the agent started the technician ticket (eval I2 after M6 step 2b, identified caller wanting to hang up). Same class as F-13 | `locales/lt/phrases.yaml` end-confirm wording, `decide/rules/head.end_confirm_answer` | fixed M6 step 4c-2 — „baigti?“ and „registruoti?“ are two questions (`dialog.end_ticket_offer`) |
| F-28 | A request mentioned in the middle of a fault call („o dar dėl sąskaitos klausimas“) is dropped: only `solve` intents become secondary problems, and there is no second ticket for a `register` intent at the closing. Found in M6 step 4c-1 when billing moved from `not_ours` to `register` | `perceive/slots.py` secondary problems, `decide/rules/closing.py`, `decide/rules/requests.py` | open — polish pass (a second request ticket at the closing) |
| F-29 | A ticket started right at identification (a request, a no-path fault) skipped the caller-name question: the contact dialogue took the turn before the ladder's last rung (eval T1 before M6 step 4c-1). Also the narrator saw the running hypothesis during the contact dialogue and invented „problema jūsų pusėje“ | `decide/rules/ticket.caller_owed`, `head.head_rule`, `reply.py`, `speak/context_card._hypothesis` | fixed M6 step 4c-1 |
| F-30 | The warm „Malonu, {name}!“ one-shot (`identity.caller_name_heard`) is not used up by a scripted reply, so it surfaces a turn later in an unrelated narrator reply (R4: after the debt news, on the offer's answer) | `speak/context_card.py` caller_name_heard, scripted replies | open — polish pass |
| F-31 | A call that drops while the end-of-call „registruoti gedimą?“ offer is unanswered still registers the fault ticket at session end („Pokalbis nutrūko…“, eval I2) — the caller had just confirmed ending. Decide in the contact-record finalizer whether an unanswered offer is a ticket or a record needing review | session end / `call_record/finalizer.py` (M6 step 5) | open — M6 step 5 |
